...
class TSNDataSet(data.Dataset):
    ...
    def _load_image(self, directory, idx, isLast=False):
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            ...
            if isLast:
                assert os.path.exists(os.path.join(self.root_path, directory, self.image_tmpl.format(idx)))
                return [Image.open(os.path.join(self.root_path, directory, self.image_tmpl.format(idx))).convert('RGB')]
            else:
                if not os.path.exists(os.path.join(self.root_path, "flow/u", directory, self.image_tmpl.format(idx))):
                    print(os.path.join(self.root_path, "flow/u", directory, self.image_tmpl.format(idx)))
                assert os.path.exists(os.path.join(self.root_path, "flow/u", directory, self.image_tmpl.format(idx)))
                assert os.path.exists(os.path.join(self.root_path, "flow/v", directory, self.image_tmpl.format(idx)))
                x_img = Image.open(os.path.join(self.root_path, "flow/u", directory, self.image_tmpl.format(idx))).convert('L')
                y_img = Image.open(os.path.join(self.root_path, "flow/v", directory, self.image_tmpl.format(idx))).convert('L')
                return [x_img, y_img]
    ...
    def __getitem__(self, index):
        ...
        if self.modality == 'RGBFlow':
            assert os.path.exists(os.path.join(self.root_path, record.path))
